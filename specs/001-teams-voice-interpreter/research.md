...
| BM-10 | 端到端首段译音 p50 / p95 | p50 ≤ 1200 ms（硬）/ ≤ 1000 ms（软）；p95 ≤ 2.0 s（2026-05-07 宪章修订自 800 ms / 1.5 s）；> 1500 ms 视为二次修订触发阈值 | 60 分钟双向同传 ≈ 200 段对话（按 spec edge case 估算） | 同 BM-1 | 超过硬阈值即阻断发布：服务栈替换、模型降档或独立宪章修订 PR |
