...
- **首段译音 p50 ≤ 1200 ms（硬阈值）/ ≤ 1000 ms（软目标）**（2026-05-07 宪章修订自 ≤ 800 ms）：BM-4 / C+α / BM-6 Piper 子段真测累加曾显示默认服务栈有望落在硬阈值内；但 Stage 5b BM-10 / BM-10D 无人值守 E2E replay 已证明当前生产顺序（ASR final → MT completed → TTS first byte）上行 p50 1720.0 ms、下行 p50 1657.9 ms，均超过硬阈值。复杂度追踪行 3 / 4 的历史处置只关闭了旧阈值与 Edge-TTS 风险，不再构成发布放行证据；下一步必须优化 TTS 启动时机 / 切段 / streaming ASR 或重审阈值。
...
| **行 8**：`tests/integration/test_ptt_command.py` 322 行，且 `test_listen_command_processes_continuous_chunks` 圈复杂度 C（>10，违反宪章 I「单文件应当 ≤ 300 行 / 圈复杂度必须 ≤ 10」）| 该测试文件覆盖 ptt / listen / duplex 命令在连续多片段输入下的全链路：模拟 BlackHole / 默认麦克风、注入 Whisper / DeepSeek / Edge-TTS 三种 fake、断言 ANSI 输出顺序、断言多个片段间的录制循环、覆盖 `--chunks` 提前结束分支；P1 流式播放需要在同一 fake 桥上保留 streaming prepare 兼容断言。单测用例需要把这些上下文一次性串好以验证真实命令行表现。 | 把这个集成测试拆成 ≥ 4 个分别 mock 一部分链路的小测试，会丢失"连续片段下命令是否还能稳定收敛"的整体覆盖，仅通过单点测试无法等价复刻；这正是该路径上唯一能在 CI 复现连续段集成行为的测试。 | 临时违例；**退出计划**：在 `live_ptt.py` 按行 6 拆分后，把当前测试改写为针对新拆分模块（`recorder` / `segmenter` / `transcriber` / `bridge`）各自单测 + 一个最小协同集成测试；目标测试文件 ≤ 300 行、圈复杂度 ≤ 10；登记日期 2026-05-05。 |
| **行 9**：`src/teams_voice_interpreter/live_say.py` 383 行（违反宪章 I「单文件应当 ≤ 300 行」）| P1 实时播放需要在原有 `say / prepare / play_prepared` 同步契约旁新增 `play_prepared_streaming`，并在实时路径支持首 PCM/首写指标、单段播放限时、上游 PCM iterator 提前关闭，以及实时 TTS 低延迟失败阈值；这些逻辑必须紧贴 `PreparedSayResult / SayResult`，否则容易破坏 `tvi say` 完整播放契约。为满足圈复杂度约束，播放限时又拆出了 `_feed_streaming_pcm / _playback_chunk` 等 helper，短期内推高了文件行数。 | 立即拆出 `live_say/streaming_playback.py` 会扩大导入边界，并要求测试同时 mock 两个模块；当前阶段优先保持 P1 行为可审阅。 | 临时违例；**退出计划**：在实时 backpressure 真测稳定后，将 `play_prepared_streaming`、PCM iterator 关闭 helper 与播放限时 helper 拆到 `live_say_streaming.py`，保留 `LiveSayBridge` 同步契约在 `live_say.py`；目标两个文件均 ≤ 300 行；登记日期 2026-05-05。 |
