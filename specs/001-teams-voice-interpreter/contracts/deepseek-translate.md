...
| 指标 | 预算 | 测量位置 |
|------|------|----------|
| 首 token 延迟 | p50 ≤ 400 ms / p95 ≤ 800 ms | `LatencySample(stage=MT_FIRST_TOKEN)` |
| 整段延迟（30–80 字英文输出） | p50 ≤ 1500 ms / p95 ≤ 2500 ms | `LatencySample(stage=MT_COMPLETED)` |
| 重试退避总耗时 | ≤ 8 s（5 次累计） | 客户端内部计时 |
