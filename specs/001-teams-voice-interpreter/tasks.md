...
- **[P]**：可并行（不同文件、无未完成依赖）
- **[Story]**：所属用户故事（US1 / US2 / US3 / US4）；Setup / Foundational / Polish 阶段无标签
...
**Checkpoint**：`make lint && make typecheck && make test` 全部 0 错误（暂无生产代码、仅空目录 + 占位 `__init__.py`）。

---

## Phase 2：Foundational（阻塞前置 — 全部用户故事的共同基础）

**目的**：核心数据模型、共享基础设施、错误规范、UX 术语表、单实例锁、设备发现 — 任一用户故事都依赖这些。

**⚠️ 关键**：本阶段未完成前，任一用户故事任务均**不得**开始。

- [ ] T010 [P] 创建 `src/teams_voice_interpreter/errors.py`：`UserFacingError` 基类（强制两段式 `what_happened` + `next_action` 字段），定义 `BlackHoleMissingError` / `AggregateDeviceMissingError` / `DeepSeekError` / `EdgeTTSError` / `WhisperError` 等子类
- [ ] T011 [P] 创建 `src/teams_voice_interpreter/glossary/strings.py`：所有用户可见字符串键的常量定义（启动 / 暂停 / 错误 / 健康状态 / 导出 等）
- [ ] T012 [P] 创建 `src/teams_voice_interpreter/glossary/i18n/zh-CN.toml`：T011 全部键的中文文案，遵循「发生了什么 + 下一步如何做」两段式
- [ ] T013 [P] 创建 `src/teams_voice_interpreter/config.py`：`Settings` Pydantic 模型 + `pydantic-settings` 加载（`~/.config/teams-voice-interpreter/config.toml` + 环境变量优先级最高 + `.env` 兜底）
- [ ] T014 [P] 创建 `src/teams_voice_interpreter/data/session.py`：`Session` + `SessionState` 状态机枚举（IDLE/STARTING/ACTIVE/PAUSED/STOPPING/STOPPED/ERRORED）
- [ ] T015 [P] 创建 `src/teams_voice_interpreter/data/transcript.py`：`TranscriptSegment`、`TranslationSegment`、`TranscriptKind` 枚举
- [ ] T016 [P] 创建 `src/teams_voice_interpreter/data/audio_segment.py`：`AudioStream`、`AudioDirection`、`SynthesizedAudioSegment`
- [ ] T017 [P] 创建 `src/teams_voice_interpreter/data/latency.py`：`LatencyStage` 枚举、`LatencySample`、`LatencySnapshot`（仅 schema，统计计算延至 T076）
- [ ] T018 [P] 创建 `src/teams_voice_interpreter/data/credential.py`：`ServiceCredential`、`ServiceKind`（密钥仅引用环境变量名）
- [ ] T019 [P] 创建 `src/teams_voice_interpreter/data/glossary.py`：`GlossaryEntry` + `glossary.toml` 加载与去重 + 200 条上限校验
- [ ] T020 [P] 创建 `src/teams_voice_interpreter/data/crash.py`：`CrashReport` + 禁字段断言（音频 / 文本 / Key / 家目录路径必须缺失）
- [ ] T021 创建 `tests/unit/data/test_session_state.py`：覆盖 `Session` 状态机所有合法 / 非法转换（依赖 T014）
- [ ] T022 [P] 创建 `tests/unit/data/test_models.py`：覆盖 T015–T020 全部模型字段校验、边界值、序列化往返（依赖 T015–T020）
- [ ] T023 创建 `src/teams_voice_interpreter/session/instance_lock.py`：基于 `fcntl.flock(LOCK_EX | LOCK_NB)` 的单实例锁（FR-026），写 PID + 启动时间 + SessionId + Web 端口；含僵尸锁检测（`kill -0 <pid>`）+ atexit 清理
- [ ] T024 创建 `tests/unit/session/test_instance_lock.py`：覆盖正常获取 / 已被持有 / 僵尸锁清理 / 多次释放幂等（依赖 T023）
- [ ] T025 [P] 创建 `src/teams_voice_interpreter/audio/routing.py`：`AudioDeviceProbe` 类含 `find_blackhole_2ch()` / `find_aggregate_with_blackhole()` / `get_default_input()` / `get_default_output()`（contracts/blackhole-coreaudio.md §2）
- [ ] T026 [P] 创建 `tests/unit/audio/test_routing.py`：mock `sounddevice.query_devices()` 返回预设设备列表，覆盖 BlackHole 缺失 / Aggregate 缺失 / 正常路径（依赖 T025）
- [ ] T027 [P] 创建 `src/teams_voice_interpreter/perf.py` 骨架：`Stopwatch` 上下文管理器 + `LatencyRecorder` 类（向 `LatencySnapshot` 写入样本）
- [ ] T028 创建 `tests/perf/fixtures/README.md`：说明 `conference-cn.wav` / `conference-en.wav` / `long-cn-2h.wav` 三份 fixture 的来源（Common Voice + LibriSpeech 商务子集）、SHA256 校验、生成脚本路径；为后续 BM 任务提供录制规范
- [ ] T029 创建 `.github/workflows/ci.yml`（或同级 GitLab CI 配置）：在 push / PR 时跑 `make lint && make typecheck && make test && make benchmark`，benchmark 任一回归阻断合并

**Checkpoint**：所有数据模型、错误基类、术语表、配置加载、单实例锁、设备发现就绪；`make test` 跑过 T021/T022/T024/T026 单元测试。所有用户故事可并行启动。

---

## Phase 3：用户故事 1 — 上行同传：用户说中文，对方听到英文（优先级 P1） 🎯 MVP
...
### US1 — 性能基线 benchmark（按 plan.md Complexity Tracking gating）
...
- [ ] T055 [P] [US1] 在 `tests/perf/test_glossary_quality.py` 中实现 BM-5（术语表注入译文质量盲测 30 对句子），结果写入 perf-report.md「术语表」段
...
- [ ] T058 [US1] 在 `tests/perf/test_first_segment_latency.py` 中实现 BM-10（端到端首段译音 p50 / p95），结果写入 perf-report.md「端到端首段」段（依赖 T044 + T052..T057）
- [ ] T059 [US1] 评审 perf-report.md 行 1 / 2 / 3：若任一 BM 命中宪章 IV 违例，按 plan.md 出口动作发起宪章修订 PR 或在 plan.md Complexity Tracking 中升级为「已批准的例外」

### US1 — 合规审计

- [ ] T060 [US1] 审计 US1 全部用户可见字符串：100% 引用 `glossary/strings.py` 共享键 + 100% 通过 `errors.UserFacingError` 两段式（宪章 III 门禁）

**Checkpoint**：US1 端到端可用；perf-report.md 已含 BM-1..6 + BM-8 + BM-10 基线；故事 1 全部 3 个验收场景在 T033 集成测试中通过。可作为独立 MVP 上线。

---

## Phase 4：用户故事 2 — 下行同传：远端说英文，用户听到中文（优先级 P1）
...
- [ ] T073 [US2] 在 `tests/perf/test_aggregate_device_jitter.py` 中实现 BM-9（Aggregate Device 同时给两个目标的 jitter ≤ 10 ms），结果写入 perf-report.md「Aggregate jitter」段
- [ ] T074 [US2] 端到端跑通 T061：US1 + US2 并行运行 60 秒不串扰（spec US2 验收场景 3）

**Checkpoint**：US1 + US2 共同构成完整双向同传闭环；perf-report.md 已含 BM-9。

---

## Phase 5：用户故事 3 — 实时状态面板（优先级 P2）

**故事目标**：用户在会议进行时通过 `http://localhost:8765` 看到运行时长、双向最近识别 / 译文、滚动 p50/p95 延迟、三服务连接健康状态；面板从事件发生到 DOM 更新 ≤ 1 秒；推送 ≥ 5 Hz。

**独立测试**：双向同传运行中打开本地 Web 控制台；任意时刻面板显示的延迟数字与外部探针测得的延迟差异 ≤ 200 ms；服务异常时 ≤ 5 秒在面板显示「非健康」+ 两段式提示。
...
- [ ] T077 [US3] 完整实现 `src/teams_voice_interpreter/perf.py` 中的 `LatencyRecorder`：滚动 60 s 窗口、各 stage p50/p95/avg/max 实时计算（依赖 T027 骨架）
...
- [ ] T080 [P] [US3] 升级 `src/teams_voice_interpreter/web/static/app.js` 加入浏览器原生 `Notification.requestPermission()` 引导 + `service_error` / `subprocess_circuit_break` 事件触发原生通知（contracts/web-control-api.md §5.3）
- [ ] T081 [US3] 审计面板字符串：100% 引用 `glossary/strings.py`；服务健康状态徽章用统一 enum（healthy / degraded / unavailable）+ 中文文案（宪章 III 门禁）

**Checkpoint**：状态面板满足 SC-008 ≤ 1 s 滞后 + FR-016 全字段；US1+US2+US3 三故事独立可用。

---

## Phase 6：用户故事 4 — 启停、暂停、设备切换、故障自愈（优先级 P3）
...
- [ ] T082 [P] [US4] 在 `tests/integration/test_supervisor.py` 中编写集成测试：覆盖 spec US4 验收场景 1（设备切换 5 s 接管）/ 2（暂停继续 2 s 恢复）/ 3（网络瞬断 30 s 重连 + 「已恢复」提示）/ 4（Whisper 子进程 kill -9 → 5 s 内 respawn + 上下文不丢）
- [ ] T083 [P] [US4] 在 `tests/unit/session/test_supervisor.py` 中编写单元测试：heartbeat 超时 3 s 视为卡死、60 s 滚动窗口 ≥ 3 次崩溃熔断、respawn 不清空 SessionId / 滚动上下文 / 术语表
...
- [ ] T089 [US4] 扩展 `src/teams_voice_interpreter/tts/edge_tts_client.py` 加入完整 401/403 token 自动刷新（最多 3 次）+ 连续 3 次失败触发 plan Complexity Tracking 行 4 降级
- [ ] T090 [US4] 扩展 `src/teams_voice_interpreter/stt/client.py` 加入 30 s 不可恢复降级（FR-019）
...
**Checkpoint**：US1+US2+US3+US4 全部独立可用；进程级故障自愈 + 匿名崩溃报告就绪。

---

## Phase 7：Polish & 横切关注点（首次运行向导 + 导出 + 长时长稳定性 + 文档收尾）
...
- [ ] T106 撰写并审定 `specs/001-teams-voice-interpreter/perf-report.md` 总体结论段：13 项 BM 全部通过 / 部分命中宪章 IV 违例时附宪章修订 PR 链接
- [ ] T107 跑通 `quickstart.md` 全流程（在新 Mac 或干净虚拟环境）：从 `pip install -e .` 到首次成功译音 ≤ 15 分钟（验证 SC-007）
- [ ] T108 [P] 全量代码审阅：`radon cc -a -nb` 验证圈复杂度 ≤ 10、单文件 ≤ 300 行；超阈值在 plan.md Complexity Tracking 登记
...
- **Phase 2（Foundational）**：依赖 Phase 1 完成 — **阻塞**全部用户故事
- **Phase 3+（用户故事）**：均依赖 Phase 2 完成
  - US1（P1）即 MVP，建议先单独完成
  - US2（P1）可在 US1 完成后开始；多人团队可与 US1 并行
  - US3（P2）可在 US1 + US2 完成后开始
  - US4（P3）可在 US3 完成后开始；部分扩展现有 US1 / US2 模块
- **Phase 7（Polish）**：依赖所选用户故事全部完成

### 用户故事依赖

- **US1（P1 / MVP）**：仅依赖 Phase 2；交付完整上行同传，可独立上线
- **US2（P1）**：仅依赖 Phase 2；多数任务为扩展 US1 已有模块支持反向；可在 US1 完成后立即开始
- **US3（P2）**：仅依赖 Phase 2；技术上独立于 US1/US2，但需要 US1/US2 的真实数据流验证面板
- **US4（P3）**：扩展 US1/US2 已有模块的容错与恢复逻辑；建议在 US1/US2 稳定后再开始

### 用户故事内部

- 测试任务（T030–T033 / T061–T062 / T075–T076 / T082–T084）**必须**先于实现任务编写并失败
- 数据模型（Phase 2）→ 服务（管线模块）→ 端点（CLI / Web）→ 集成测试通过
- perf benchmark（T052–T058 / T073 / T100–T103）必须在故事 Checkpoint 前完成

### 并行机会

- Phase 1 中 T003 / T004 / T005 / T006 / T007 / T008 可并行
- Phase 2 中 T010..T020 + T025 + T027 可并行（不同文件）
- Phase 3 US1 中 T030..T033（测试）可并行；T034..T037 + T039..T040 + T042（实现）可并行；T046..T047 + T050..T051 可并行；T052..T057（perf）可并行
- Phase 4 US2 中 T061..T062 可并行；T063..T064 可并行
- Phase 5 US3 中 T075..T076 可并行；T079..T080 可并行
- Phase 6 US4 中 T082..T084 可并行；T085..T086 可并行
- 多人团队：Phase 2 完成后 US1 / US2 / US3 / US4 可由不同人并行推进

---

## 并行执行示例

### Phase 2 Foundational 并行启动（单人单 IDE）

```bash
# 同时打开以下文件并独立完成（互不干扰）：
$EDITOR src/teams_voice_interpreter/errors.py             # T010
$EDITOR src/teams_voice_interpreter/glossary/strings.py   # T011
$EDITOR src/teams_voice_interpreter/data/session.py       # T014
$EDITOR src/teams_voice_interpreter/data/transcript.py    # T015
$EDITOR src/teams_voice_interpreter/data/audio_segment.py # T016
$EDITOR src/teams_voice_interpreter/data/latency.py       # T017
$EDITOR src/teams_voice_interpreter/data/credential.py    # T018
$EDITOR src/teams_voice_interpreter/data/glossary.py      # T019
$EDITOR src/teams_voice_interpreter/data/crash.py         # T020
```
...
3. 完成 Phase 3（US1 上行同传）
4. **暂停并验证**：T033 集成测试通过；T058 BM-10 实测 + T059 评审 perf-report.md
5. 若 BM-10 命中宪章 IV 违例：触发宪章 PR 或 plan Complexity Tracking 升级；否则即可作为 MVP 上线

### 增量交付（推荐节奏）

1. **Sprint 1**：Setup + Foundational 完成 → 基础就绪
2. **Sprint 2**：US1 完成 → 单向上行同传 MVP 上线
3. **Sprint 3**：US2 完成 → 双向同传完整闭环
4. **Sprint 4**：US3 完成 → 完整状态面板可观测
5. **Sprint 5**：US4 完成 → 故障自愈生产级
6. **Sprint 6**：Polish（向导 + 导出 + 长时长 perf + README + 全量审阅）→ v1.0 RC

### 并行团队策略

3 人团队推荐分工：

1. 全员合力完成 Setup + Foundational（约 1.5 个 Sprint）
2. Foundational 完成后：
   - **Dev A**：US1 完整管线 + perf benchmark（约 2 Sprint）
   - **Dev B**：US2 反向扩展（在 US1 模块上扩展，需 US1 接口稳定后开始）+ Polish 中的导出（FR-027）
   - **Dev C**：US3 + US4（容错与可观测，相对独立于上下行管线本身）
3. Polish 阶段全员合力完成长时长 perf + 文档审阅 + initial commit
...
- perf benchmark 强制：每个故事的 Checkpoint 前必须有对应 BM 任务（宪章 IV）
- 共享术语表 + 错误两段式合规校验是每个故事 Checkpoint 的硬门禁（宪章 III）
- 提交节奏：建议每完成一个任务或一个逻辑组就 commit 一次
- 每个 Checkpoint 都可暂停验证、独立 demo
